#include "engine.h"
#include "ship.h"
#include "enemy.h"
#include "enemy_diagonal.h"
#include "enemy_diagonal_stationary.h"
#include "enemy_diagonal_stationary_fwdsprd.h"
#include "enemy_cargo.h"
#include "powerup_double_shot.h"
#include "powerup_quad_spread_shot.h"
#include "player_projectile.h"
#include "enemy_projectile.h"
#include "projectile_manager.h"
#include <SDL2/SDL.h>
#include <SDL2/SDL_image.h>
#include <time.h>
#include <unistd.h>

#define RES_X 800
#define RES_Y 600
#define BPP 32

int ID_PLAYER_SHIP = 1;
int ID_ENEMY_SHIP = 2;
int ID_POWERUP_DOUBLE_SHOT = 100;
int ID_POWERUP_QUAD_SPREAD_SHOT = 101;
int ID_PLAYER_SHOT = 1000;
int ID_ENEMY_SHOT = 1001;
int SHOT_PHYS_DELAY = 2500000;

#define NUM_SHOTS 40
#define NUM_SHOTS_ENEMY 40

#define ENEMY_SET_COUNT 4

#define MAX_ENEMY_SLOTS 10

bool game_over = false;

void activate_enemy_set(engine_obj_list *enemy_slots[MAX_ENEMY_SLOTS], int set_id, engine *eng, projectile_manager *enemy_shot_mngr, powerup_double_shot *powerup_double_shot_obj, powerup_quad_spread_shot *powerup_quad_spread_shot_obj)
{
    switch (set_id)
    {
        case 0:
            delete enemy_slots[0]->obj;
            delete enemy_slots[1]->obj;
            delete enemy_slots[2]->obj;

            enemy_slots[0]->obj = new enemy(eng, enemy_shot_mngr);
            enemy_slots[0]->obj->init();

            enemy_slots[1]->obj = new enemy_diagonal(eng, enemy_shot_mngr);
            enemy_slots[1]->obj->init();

            enemy_slots[2]->obj = new enemy_cargo(eng, enemy_shot_mngr);
            enemy_slots[2]->obj->init();
            ((enemy*)(enemy_slots[2]->obj))->drop_powerup = powerup_double_shot_obj;

            enemy_slots[0]->obj->pos_x = 150;
            enemy_slots[0]->obj->pos_y = enemy_slots[0]->obj->size_y*-1;
            enemy_slots[0]->obj->step_x = 1;
            enemy_slots[0]->obj->step_y = 1;
            enemy_slots[0]->obj->draw_active = true;
            enemy_slots[0]->obj->phys_active = true;
            enemy_slots[0]->obj->move_x_last = {0, 0};
            enemy_slots[0]->obj->move_y_last = {0, 0};
            enemy_slots[1]->obj->pos_x = 300;
            enemy_slots[1]->obj->pos_y = enemy_slots[1]->obj->size_y*-2;
            enemy_slots[1]->obj->step_x = -1;
            enemy_slots[1]->obj->step_y = 1;
            enemy_slots[1]->obj->draw_active = true;
            enemy_slots[1]->obj->phys_active = true;
            enemy_slots[1]->obj->move_x_last = {0, 0};
            enemy_slots[1]->obj->move_y_last = {0, 0};
            enemy_slots[2]->obj->pos_x = 650;
            enemy_slots[2]->obj->pos_y = enemy_slots[2]->obj->size_y*-3;
            enemy_slots[2]->obj->step_x = 0;
            enemy_slots[2]->obj->step_y = 1;
            enemy_slots[2]->obj->draw_active = true;
            enemy_slots[2]->obj->phys_active = true;
            enemy_slots[2]->obj->move_x_last = {0, 0};
            enemy_slots[2]->obj->move_y_last = {0, 0};

            break;

        case 1:
            delete enemy_slots[0]->obj;
            delete enemy_slots[1]->obj;
            delete enemy_slots[2]->obj;

            enemy_slots[0]->obj = new enemy_diagonal(eng, enemy_shot_mngr);
            enemy_slots[0]->obj->init();

            enemy_slots[1]->obj = new enemy_diagonal(eng, enemy_shot_mngr);
            enemy_slots[1]->obj->init();

            enemy_slots[2]->obj = new enemy_diagonal(eng, enemy_shot_mngr);
            enemy_slots[2]->obj->init();

            enemy_slots[0]->obj->pos_x = 150;
            enemy_slots[0]->obj->pos_y = enemy_slots[0]->obj->size_y*-1;
            enemy_slots[0]->obj->step_x = 1;
            enemy_slots[0]->obj->step_y = 1;
            enemy_slots[0]->obj->draw_active = true;
            enemy_slots[0]->obj->phys_active = true;
            enemy_slots[0]->obj->move_x_last = {0, 0};
            enemy_slots[0]->obj->move_y_last = {0, 0};
            enemy_slots[1]->obj->pos_x = 300;
            enemy_slots[1]->obj->pos_y = enemy_slots[1]->obj->size_y*-3;
            enemy_slots[1]->obj->step_x = -1;
            enemy_slots[1]->obj->step_y = 1;
            enemy_slots[1]->obj->draw_active = true;
            enemy_slots[1]->obj->phys_active = true;
            enemy_slots[1]->obj->move_x_last = {0, 0};
            enemy_slots[1]->obj->move_y_last = {0, 0};
            enemy_slots[2]->obj->pos_x = 100;
            enemy_slots[2]->obj->pos_y = enemy_slots[2]->obj->size_y*-5;
            enemy_slots[2]->obj->step_x = -1;
            enemy_slots[2]->obj->step_y = 1;
            enemy_slots[2]->obj->draw_active = true;
            enemy_slots[2]->obj->phys_active = true;
            enemy_slots[2]->obj->move_x_last = {0, 0};
            enemy_slots[2]->obj->move_y_last = {0, 0};

            break;

        case 2:
            delete enemy_slots[0]->obj;
            delete enemy_slots[1]->obj;

            enemy_slots[0]->obj = new enemy_diagonal_stationary(eng, enemy_shot_mngr);
            enemy_slots[0]->obj->init();

            enemy_slots[1]->obj = new enemy_diagonal_stationary(eng, enemy_shot_mngr);
            enemy_slots[1]->obj->init();
            ((enemy*)(enemy_slots[1]->obj))->drop_powerup = powerup_quad_spread_shot_obj;

            enemy_slots[0]->obj->pos_x = 150;
            enemy_slots[0]->obj->pos_y = enemy_slots[0]->obj->size_y*-1;
            enemy_slots[0]->obj->step_x = 0;
            enemy_slots[0]->obj->step_y = 1;
            enemy_slots[0]->obj->draw_active = true;
            enemy_slots[0]->obj->phys_active = true;
            enemy_slots[0]->obj->move_x_last = {0, 0};
            enemy_slots[0]->obj->move_y_last = {0, 0};
            enemy_slots[1]->obj->pos_x = 550;
            enemy_slots[1]->obj->pos_y = enemy_slots[0]->obj->size_y*-4;
            enemy_slots[1]->obj->step_x = 0;
            enemy_slots[1]->obj->step_y = 1;
            enemy_slots[1]->obj->draw_active = true;
            enemy_slots[1]->obj->phys_active = true;
            enemy_slots[1]->obj->move_x_last = {0, 0};
            enemy_slots[1]->obj->move_y_last = {0, 0};

            break;

        case 3:
            enemy_slots[0]->obj = new enemy_diagonal_stationary_fwdsprd(eng, enemy_shot_mngr);
            enemy_slots[0]->obj->init();

            enemy_slots[1]->obj = new enemy_diagonal_stationary_fwdsprd(eng, enemy_shot_mngr);
            enemy_slots[1]->obj->init();

            enemy_slots[0]->obj->pos_x = 150;
            enemy_slots[0]->obj->pos_y = enemy_slots[0]->obj->size_y*-1;
            enemy_slots[0]->obj->step_x = 0;
            enemy_slots[0]->obj->step_y = 1;
            enemy_slots[0]->obj->draw_active = true;
            enemy_slots[0]->obj->phys_active = true;
            enemy_slots[0]->obj->move_x_last = {0, 0};
            enemy_slots[0]->obj->move_y_last = {0, 0};
            enemy_slots[1]->obj->pos_x = 550;
            enemy_slots[1]->obj->pos_y = enemy_slots[0]->obj->size_y*-4;
            enemy_slots[1]->obj->step_x = 0;
            enemy_slots[1]->obj->step_y = 1;
            enemy_slots[1]->obj->draw_active = true;
            enemy_slots[1]->obj->phys_active = true;
            enemy_slots[1]->obj->move_x_last = {0, 0};
            enemy_slots[1]->obj->move_y_last = {0, 0};

            break;
    }
}

void shooter()
{
    bool quit = false;
    bool left = false;
    bool right = false;
    bool up = false;
    bool down = false;
    bool fire = false;

    bool fired = false;

    int active_enemy_set = 0;
    bool init_enemy_set = false;

    SDL_Event input;

    char *base_path = SDL_GetBasePath();

    chdir(base_path);
    free(base_path);

    engine *eng = new engine("SDL SHOOTER", RES_X, RES_Y, BPP);

    // Game objects
    projectile_manager *player_shot_mngr = new projectile_manager();
    projectile_manager *enemy_shot_mngr = new projectile_manager();
    ship *ship_obj = new ship(eng, player_shot_mngr);
    player_projectile *shots[NUM_SHOTS];
    enemy_projectile *enemy_shots[NUM_SHOTS_ENEMY];
    powerup_double_shot *powerup_double_shot_obj;
    powerup_quad_spread_shot *powerup_quad_spread_shot_obj;

    engine_obj_list *enemy_slots[MAX_ENEMY_SLOTS];
    for (int i = 0; i < MAX_ENEMY_SLOTS; i++) {
        enemy_slots[i] = eng->add_object(new engine_obj());
    }

    powerup_double_shot_obj = new powerup_double_shot(eng);
    powerup_double_shot_obj->init();
    eng->add_object(powerup_double_shot_obj);

    powerup_quad_spread_shot_obj = new powerup_quad_spread_shot(eng);
    powerup_quad_spread_shot_obj->init();
    eng->add_object(powerup_quad_spread_shot_obj);

    eng->add_object(ship_obj);
    ship_obj->init();

    activate_enemy_set(enemy_slots, active_enemy_set, eng, enemy_shot_mngr, powerup_double_shot_obj, powerup_quad_spread_shot_obj);

    for (int shot_count = 0; shot_count < NUM_SHOTS; shot_count++) {
        shots[shot_count] = new player_projectile();
        shots[shot_count]->init();
        eng->add_object(shots[shot_count]);
        player_shot_mngr->add_object(shots[shot_count]);
    }

    for (int enemy_shot_count = 0; enemy_shot_count < NUM_SHOTS_ENEMY; enemy_shot_count++) {
        // Shots
        enemy_shots[enemy_shot_count] = new enemy_projectile();
        enemy_shots[enemy_shot_count]->init();
        eng->add_object(enemy_shots[enemy_shot_count]);
        enemy_shot_mngr->add_object(enemy_shots[enemy_shot_count]);
    }

    // Main loop
    while (quit==false)
    {
        // Read inputs
        while (SDL_PollEvent(&input))
        {
            switch (input.type)
            {
                case SDL_KEYDOWN:
                    switch (input.key.keysym.sym)
                        {
                            case SDLK_LEFT:
                                left = true;
                                break;
                            case SDLK_RIGHT:
                                right = true;
                                break;
                            case SDLK_UP:
                                up = true;
                                break;
                            case SDLK_DOWN:
                                down = true;
                                break;
                            case SDLK_SPACE:
                                fire = true;
                                break;
                            case SDLK_q:
                                quit = true;
                                break;
                        }
                        break;
                case SDL_KEYUP:
                    switch (input.key.keysym.sym)
                        {
                            case SDLK_LEFT:
                                left = false;
                                break;
                            case SDLK_RIGHT:
                                right = false;
                                break;
                            case SDLK_UP:
                                up = false;
                                break;
                            case SDLK_DOWN:
                                down = false;
                                break;
                            case SDLK_SPACE:
                                fire = false;
                                break;
                        }
            }
        }

        if (!game_over) {
            ship_obj->step_x = 0;
            ship_obj->step_y = 0;
            if (left) { ship_obj->step_x = -1; }
            if (right) { ship_obj->step_x = 1; }
            if (up) { ship_obj->step_y = -1; }
            if (down) { ship_obj->step_y = 1; }

            if (fire) {
                if (!fired) {
                    fired = true;
                    ship_obj->fire();
                }
            } else {
                fired = false;
            }

            init_enemy_set = true;

            for (int i = 0; i < MAX_ENEMY_SLOTS; i++) {
                if (enemy_slots[i]->obj->draw_active) {
                    init_enemy_set = false;
                }
            }

            if (init_enemy_set && ++active_enemy_set == ENEMY_SET_COUNT) {
                active_enemy_set = 0;
            }

            if (init_enemy_set) {
                activate_enemy_set(enemy_slots, active_enemy_set, eng, enemy_shot_mngr, powerup_double_shot_obj, powerup_quad_spread_shot_obj);
            }

            // Redraw screen
            eng->step();
        }
    }

    SDL_Quit();

    delete ship_obj;
    delete player_shot_mngr;
    delete enemy_shot_mngr;
    delete powerup_double_shot_obj;
    delete powerup_quad_spread_shot_obj;

    for (int shot_count = 0; shot_count < NUM_SHOTS; shot_count++) {
        delete shots[shot_count];
    }

    for (int enemy_shot_count = 0; enemy_shot_count < NUM_SHOTS_ENEMY; enemy_shot_count++) {
        delete enemy_shots[enemy_shot_count];
    }

    for (int i = 0; i < MAX_ENEMY_SLOTS; i++) {
        delete enemy_slots[i]->obj;
    }

    delete eng;

    return;
}

int main (int argc, char *argv[])
{
    shooter();
    return 0;
}

